...
def create_figures_func(inputs, pprint, aspect_ratio_array,
    ...
    if savefig: plt.savefig('particle_initial_conditions_RZphi.pdf')
    ...
    for i, minor_radius in enumerate(inputs.minor_radius_array):
        ...
        label_legends=[f'SIMPLE A={aspect_ratio_array[i]:.2f}',f'gyronimo A={aspect_ratio_array[i]:.2f}']
        plt.plot(orbit_simple_solution_array[i][:,0], np.sqrt(orbit_simple_solution_array[i][:,1])*minor_radius, '--', label=label_legends[0])
        plt.plot(orbit_gyronimo_solution_array[i][:,0], np.sqrt(orbit_gyronimo_solution_array[i][:,1])*minor_radius, ':', label=label_legends[1])
    plt.plot(orbit_nearaxis_solution[:,0], orbit_nearaxis_solution[:,1], label='Near-Axis')
    plt.xlabel('time')
    plt.ylabel(r'$r_{\mathrm{near-axis}}$')
    plt.legend()
    if savefig: plt.savefig('r_nearaxis_comparison.pdf')

    SIMPLE_VELOCITY_SIGN = -1
    fig, ax = plt.subplots()
    for i, minor_radius in enumerate(inputs.minor_radius_array):
        # if i==0: label_legends=['SIMPLE','gyronimo']
        # else: label_legends=['_nolegend_','_nolegend_']
        label_legends=[f'SIMPLE A={aspect_ratio_array[i]:.2f}',f'gyronimo A={aspect_ratio_array[i]:.2f}']
        plt.plot(orbit_simple_solution_array[i][:,0], SIMPLE_VELOCITY_SIGN*orbit_simple_solution_array[i][:,7], '--', label=label_legends[0])
        plt.plot(orbit_gyronimo_solution_array[i][:,0], orbit_gyronimo_solution_array[i][:,7], ':', label=label_legends[1])
    plt.plot(orbit_nearaxis_solution[:,0], orbit_nearaxis_solution[:,7], label='Near-Axis')
    plt.xlabel('time')
    plt.ylabel(r'$v_\parallel$')
    plt.legend()
    if savefig: plt.savefig('vparallel_nearaxis_comparison.pdf')
    ...
    for i, minor_radius in enumerate(inputs.minor_radius_array):
        ...
        # else: label_legends=['_nolegend_','_nolegend_']
        label_legends=[f'SIMPLE A={aspect_ratio_array[i]:.2f}',f'gyronimo A={aspect_ratio_array[i]:.2f}']
        # plt.plot(orbit_simple_solution_array[i][:,0], orbit_simple_solution_array[i][:,6], '--', label=label_legends[0])
        plt.plot(orbit_gyronimo_solution_array[i][:,0], orbit_gyronimo_solution_array[i][:,6], ':', label=label_legends[1])
    plt.plot(orbit_nearaxis_solution[:,0], orbit_nearaxis_solution[:,6], label='Near-Axis')
    plt.xlabel('time')
    plt.ylabel(r'$|B|$')
    plt.legend()
    if savefig: plt.savefig('modB_nearaxis_comparison.pdf')
    ...
    for i, minor_radius in enumerate(inputs.minor_radius_array):
        ...
        # else: label_legends=['_nolegend_','_nolegend_']
        label_legends=[f'SIMPLE A={aspect_ratio_array[i]:.2f}',f'gyronimo A={aspect_ratio_array[i]:.2f}']
        plt.plot(orbit_simple_solution_array[i][:,0], orbit_simple_solution_array[i][:,1], '--', label=label_legends[0])
        plt.plot(orbit_gyronimo_solution_array[i][:,0], orbit_gyronimo_solution_array[i][:,1], ':', label=label_legends[1])
    plt.xlabel('time')
    plt.ylabel(r'$s=\psi/\psi_a$')
    plt.legend()
    if savefig: plt.savefig('s_nearaxis_comparison.pdf')
